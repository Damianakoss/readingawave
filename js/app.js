async function setup() {
    ...
    try {
        ...
        if (response && (response.status >= 300 || response.status < 200)) {
            ...
            errorContext.description = `Check app.js to see what file it's trying to load.`;
        }
    ...
    // Log messages from outports straight to our text element
    attachOutports(device);

    // Resume audio on a tap anywhere on the page
...
function makeInportForm(device) {
    ...
    if (inports.length === 0) {
        ...
        inportForm.onsubmit = (ev) => {
            // Do this or else the page will reload MEMORY TEST
            ev.preventDefault();
...
function attachOutports(device) {
    device.messageEvent.subscribe((ev) => {
        if (ev.tag !== "output") return;

        // Decode ASCII character codes into text, like itoa
        const codes = Array.isArray(ev.payload) ? ev.payload : [ev.payload];
        const text = codes.map(code => String.fromCharCode(code)).join("");

        document.getElementById("rnbo-console-readout").innerText = text;
    });
}
